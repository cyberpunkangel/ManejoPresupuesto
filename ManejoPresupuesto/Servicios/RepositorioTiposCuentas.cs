...
namespace ManejoPresupuesto.Servicios
{
    ...
    public class RepositorioTiposCuentas: IRepositorioTiposCuentas
    {
        ...
        {
            using var connection = new SqlConnection(connectionString);
            var tiposCuentas = await connection.QueryAsync<TipoCuenta>(@"SELECT Id, Nombre, Orden
                                                                FROM TiposCuentas
                                                                WHERE UsuarioId = @UsuarioId
                                                                ORDER BY Orden", new { usuarioId });

            foreach (var tipoCuenta in tiposCuentas)
            {
                // Validación y limpieza adicional del campo "Nombre"
                tipoCuenta.Nombre = tipoCuenta.Nombre.Trim();
            }
